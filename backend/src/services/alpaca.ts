...
export interface AlpacaService {
  ...
  /**
   * Submit a market/day order. WRITE — moves money in a real (paper or live)
   * Alpaca account. Callers must gate this behind `env.RUNNER_DRY_RUN`; the
   * adapter itself does not know about the dry-run flag and will submit
   * whatever it's asked to.
   */
  submitOrder(symbol: string, qty: number, side: 'buy' | 'sell'): Promise<AlpacaOrderSummary>;
  /** Poll a previously submitted order by id — fill status/price. */
  getOrder(orderId: string): Promise<AlpacaOrderSummary>;
}
...
class MockAlpacaService implements AlpacaService {
  ...
  // In-memory registry so getOrder() can look up what submitOrder() just
  // "filled" within the same process lifetime. Mock only — no persistence
  // across restarts, which is fine since this path only exists for local
  // dev/tests, never a real broker.
  private mockOrders = new Map<string, AlpacaOrderSummary>();

  async submitOrder(symbol: string, qty: number, side: 'buy' | 'sell'): Promise<AlpacaOrderSummary> {
    const id = `mock-${symbol}-${side}-${this.mockOrders.size}`;
    const now = new Date().toISOString();
    const order: AlpacaOrderSummary = {
      id,
      symbol,
      side,
      qty,
      filledQty: qty,
      filledAvgPrice: seededClose(symbol, new Date()),
      status: 'filled',
      createdAt: now,
      filledAt: now,
    };
    this.mockOrders.set(id, order);
    return order;
  }

  async getOrder(orderId: string): Promise<AlpacaOrderSummary> {
    const order = this.mockOrders.get(orderId);
    if (!order) throw new Error(`Mock order not found: ${orderId}`);
    return order;
  }
}
