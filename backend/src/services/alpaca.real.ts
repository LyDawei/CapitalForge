/**
 * Real Alpaca market-data + order-execution adapter — TODO.md item #3.
 *
 * Phase B (order execution — `submitOrder`/`getOrder`) is now implemented,
 * originally copied from V1's real adapter (`packages/engine`, since removed)
 * and adapted to this file's tradingGet-style conventions. It is NOT
 * self-gating: the caller
 * (cycleRunner) is responsible for checking `env.RUNNER_DRY_RUN` before ever
 * calling `submitOrder` — this adapter will submit whatever it's told to.
 *
 * What this does cover:
 *   - getDailyBars / getBarsAfter with `next_page_token` pagination
 *   - getCurrentPrice: latest trade → fallback to latest bar
 *   - submitOrder / getOrder — market, day-order, against the trading API
...
export class RealAlpacaService implements AlpacaService {
  ...
  /**
   * WRITE. Submits a market/day order. Not wrapped in fetchWithAudit — same
   * reasoning as getAccount/getPositions above: this isn't an agent input,
   * it's the runner acting on a decision already made. The caller
   * (cycleRunner) is responsible for the RUNNER_DRY_RUN gate; this method
   * has no opinion and will submit whatever it's asked to.
   */
  async submitOrder(symbol: string, qty: number, side: 'buy' | 'sell'): Promise<AlpacaOrderSummary> {
    await this.limiter.take();
    ...
    if (!res.ok) {
      const body = await res.text().catch(() => '');
      throw new Error(`Alpaca order submission ${res.status}: ${body || res.statusText}`);
    }
    return this.mapOrder(await res.json());
  }

  /** Poll a previously submitted order by id — fill status/price. */
  async getOrder(orderId: string): Promise<AlpacaOrderSummary> {
    const o = await this.tradingGet<any>(`/v2/orders/${orderId}`);
    return this.mapOrder(o);
  }

  private mapOrder(o: any): AlpacaOrderSummary {
    return {
      id: o.id,
      symbol: o.symbol,
      side: o.side,
