    ...
    if (headTraderResult?.parsed) {
      ...
      if (action !== 'HOLD') {
        ...
        if (action === 'BUY' || action === 'SELL') {
          await submitOrderIfArmed(tradePlan.id, symbol, action, shares);
        }
      }
    }
...
/**
 * Phase B — real order submission, gated by RUNNER_DRY_RUN (default true).
 * Dry-run logs what would have been submitted and returns without touching
 * the broker. A submission failure is logged and swallowed, same pattern as
 * the pre-cycle settler and feed fetches above: the TradePlan (the audit
 * record) is already persisted, so a broker outage shouldn't fail the cycle.
 */
async function submitOrderIfArmed(
  tradePlanId: string,
  symbol: string,
  action: 'BUY' | 'SELL',
  shares: number,
  ...
  if (shares <= 0) {
    // eslint-disable-next-line no-console
    console.log(`[order] ${symbol} ${action}: 0 shares computed, skipping submission.`);
    return;
  }
  const side = action === 'BUY' ? 'buy' : 'sell';
  if (env.RUNNER_DRY_RUN) {
    // eslint-disable-next-line no-console
    console.log(`[order] DRY RUN: would submit ${side} ${shares} ${symbol}.`);
    return;
  }
  try {
    const order = await getAlpacaService().submitOrder(symbol, shares, side);
    await prisma.tradePlan.update({
      where: { id: tradePlanId },
      data: {
        brokerOrderId: order.id,
        brokerOrderStatus: order.status,
        brokerFilledQty: order.filledQty,
        brokerFilledAvgPrice: order.filledAvgPrice,
        brokerSubmittedAt: new Date(),
      },
    });
    // eslint-disable-next-line no-console
    console.log(`[order] submitted ${side} ${shares} ${symbol} -> broker order ${order.id} (${order.status})`);
  } catch (err: any) {
    // eslint-disable-next-line no-console
    console.warn(`[order] submission failed for ${symbol} ${side} ${shares}: ${err.message}`);
  }
}
