# Postgres in Docker: runbook & troubleshooting

This project keeps **only Postgres** in a container (`capitalforge-v2-db`). The
...
`%APPDATA%\Docker\settings-store.json`). 2 GB was too tight and contributed to
earlier Docker Desktop instability during a heavy build; 4 GB has been stable
since.
