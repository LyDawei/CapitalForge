# CapitalForge — Agent Audit Console

Focused on **observing, auditing, and improving** the AI agents that make
trading decisions. Not a Docker-first project — every piece runs locally with
Node, Postgres, and `npm`.

> **Postgres runs in Docker** (`docker compose up -d postgres`). The backend
> and frontend still run locally via `npm run dev`; only the database is
> containerized, so data lives in the managed `cf_v2_pgdata` volume instead of a
> local Postgres install. The container (`capitalforge-v2-db`) owns host port
> 5432. Operational commands and a full troubleshooting runbook live in
> **[`DOCKER.md`](DOCKER.md)**.
...
Eight specialists run in parallel per cycle, feeding a head-trader-and-decision-layer
pattern, plus a post-cycle critic aimed at keeping the head trader honest:
...
```bash
# register + activate a new version (deactivates prior ones)
...
```bash
# real LLM (MODE=paper + OPENAI_API_KEY). ~$0.05–0.09 per cycle.
