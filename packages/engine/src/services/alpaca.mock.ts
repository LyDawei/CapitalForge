 ...
 */
function buildBarForEpoch(symbol: string, epochDay: number, date: Date): DailyBar {
...
function epochDaysFor(date: Date): number {
...
function generateBarsAfter(symbol: string, fromDate: string, limit: number): DailyBar[] {
  const bars: DailyBar[] = [];
  ...
  while (bars.length < limit) {
    ...
    if (epochDaysFor(cursor) - epochDaysFor(start) > limit * 3 + 30) break;
  }
  return bars;
}
...
export class MockAlpacaService implements AlpacaService {
  ...
  async getBarsAfter(symbol: string, fromDate: string, limit: number): Promise<DailyBar[]> {
