...
import { createOrchestrator, OrchestratorAPI } from './core/orchestrator';
import { createOrchestratorV2 } from './v2/engine/orchestratorV2';
import { disconnectPrisma } from './services/db';
import dotenv from 'dotenv';
import path from 'path';
// Load root-level .env first (monorepo source of truth), then a local override if it exists.
...
async function runCycle(orchestrator: OrchestratorAPI, symbols: string[]): Promise<void> {
  try {
...
async function main(): Promise<void> {
  ...
  // Create orchestrator (engine version routed by config.engineVersion)
  console.log(`  Engine version: ${config.engineVersion}`);
  const orchestrator: OrchestratorAPI =
    config.engineVersion === 'v2'
      ? createOrchestratorV2(config, alpaca, llm, news)
      : createOrchestrator(config, alpaca, llm, news);
